...
const dispatchChange = (conn, config, content, params = {}) => {
  ...
  return fetch(
    conn.request(
      ...
      `${config.resource}${encodeQueryString(queryParams)}`
    ),
    {
      method: 'POST',
      headers,
...
const model = (conn, database, options = {}, params = {}) => {
  ...
  return fetch(conn.request(database, `model${encodeQueryString(params)}`), {
    headers,
  }).then(httpBody);
};
...
const clear = (conn, database, transactionId, params = {}) => {
  ...
  return fetch(
    conn.request(
      database,
      transactionId,
      `clear${encodeQueryString(queryParams)}`
    ),
    {
      method: 'POST',
      headers,
...
const exportData = (
  ...
  const fetchPromise = fetch(
    conn.request(database, `export${encodeQueryString(queryParams)}`),
    { headers }
  );
