...
(function(factory) {
  ...
  if (isNode) {
    ...
    Connection.prototype._httpRequest = function(options, callback) {
      ...
      rest.request(req_url, reqJSON).on('complete', fnResponseHandler);
    };
  } else {
    ...
    Connection.prototype._base64Encode = typeof btoa == 'function'
      ? function(x) {
          return btoa(x);
        }
      : null;
    ...
    Connection.prototype._httpRequest = function(options, callback) {
      ...
      if (this.credentials) {
        ...
        if (this._base64Encode) {
          ...
          throw new Error(
            'Your browser does not support btoa() natively.\n' +
      ...
      req_url = !_.isEmpty(params) ? req_url + '?' + $.param(params) : req_url;
      ...
      $.ajax(ajaxSettings)
        .done(function(data, status, jqXHR) {
          ...
          if (
            data &&
            (data.hasOwnProperty('@id') || data.hasOwnProperty('@context'))
          ) {
  ...
  Connection.prototype.exportDB = function(options, callback) {
    ...
    reqOptions.params['graph-uri'] =
      options.graphUri || 'tag:stardog:api:context:all';
  ...
  Connection.prototype.listDBs = function(options, callback) {
    ...
    this._httpRequest(
      ...
      typeof options === 'function'
        ? options /* no options passed, so this is the callback */
  ...
  Connection.prototype.createDB = function(options, callback) {
    ...
    this._httpRequest(reqOptions, data => {
      console.log(data);
    });
  };
  ...
  Connection.prototype.createUser = function(options, callback) {
    var reqOptions = {
      ...
      msgBody: {
        ...
        password: options.password.split(''),
      },
      isJsonBody: true,
      params: options.params || '',
    };

    // If it's set to something, use that
    if (typeof options.superuser === 'boolean') {
      reqOptions.msgBody.superuser = options.superuser;
    } else {
      // Default to false
      reqOptions.msgBody.superuser = false;
    }
